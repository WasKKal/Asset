...
for i, preset in ipairs(PRESET_PAGES) do
    ...
    if preset.unsafe then
        local warnIcon = GetIcon("triangle-alert", UDim2.new(0, 12, 0, 12))
        if warnIcon then
...
local LINE_NUMBER_MIN_WIDTH = 28
local LINE_NUMBER_HPAD = 12
local lineNumberWidth = LINE_NUMBER_MIN_WIDTH
...
local function updateLineNumberWidth()
    ...
    if codePageBreaks and currentCodePage and #codePageBreaks > 0 and currentCodePage <= #codePageBreaks then
        lastLine = (codePageBreaks[currentCodePage] or 1) + 4999
    end
    local sample = tostring(lastLine)
    ...
    local w
    if ok and size then
        w = math.ceil(size.X) + LINE_NUMBER_HPAD
    else
