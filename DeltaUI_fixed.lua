...
function LoadLucide()
    ...
    if not content or content == "" then
        local moduleUrls = {
            "https://cdn.jsdelivr.net/gh/WasKKal/Asset@latest/lucide/init.lua",
...
local uiVersion = "1.0.4"
local UI_VERSION = "1.0.4"
...
local PRESET_PAGES = {
    {
        ...
        icon = "blocks",
        url = "https://raw.githubusercontent.com/WasKKal/Asset/master/deltaui/coding_blocks.lua",
        unsafe = true,
    },
    {
        name = "deobfuscator",
        ...
        icon = "shield-check",
        url = "https://raw.githubusercontent.com/WasKKal/Asset/master/deltaui/deobfuscator.lua",
        unsafe = true,
    },
}
...
local function compareVersions(v1, v2)
    ...
    v2 = tostring(v2 or "0")
    local parts1 = {}
    for n in v1:gmatch("%d+") do table.insert(parts1, tonumber(n) or 0) end
    local parts2 = {}
    for n in v2:gmatch("%d+") do table.insert(parts2, tonumber(n) or 0) end
...
-- 页面信息来源：页面代码里的 DeltaPageInfo 变量
-- 约定：顶格书写（行首无空格）、扁平表、值为字符串/布尔/数字；不支持嵌套表，值里不要出现 } 或 "
-- 只读取 name/title/icon/version/dataFolder；unsafe 与 url 一律不认页面声明，避免页面自己提权
local PAGE_INFO_FIELDS = { "title", "icon", "version", "dataFolder" }

local function extractPageInfo(code)
    ...
    for i, line in ipairs(lines) do
        local rest = line:match("^local%s+DeltaPageInfo%s*=%s*{(.*)$") or line:match("^DeltaPageInfo%s*=%s*{(.*)$")
        if rest ~= nil then
            if not rest:find("}") then
                for j = i + 1, math.min(i + 60, #lines) do
                    rest = rest .. "\n" .. lines[j]
                    if rest:find("}") then break end
                end
            end
            local body = rest:match("^(.-)}") or rest
            local info = {}
            for key, val in body:gmatch('([%w_]+)%s*=%s*"([^"]*)"') do info[key] = val end
            for key, val in body:gmatch("([%w_]+)%s*=%s*'([^']*)'") do
                if info[key] == nil then info[key] = val end
            end
            for key in body:gmatch('([%w_]+)%s*=%s*true') do
                if info[key] == nil then info[key] = true end
            end
            for key in body:gmatch('([%w_]+)%s*=%s*false') do
                if info[key] == nil then info[key] = false end
            end
            for key, val in body:gmatch('([%w_]+)%s*=%s*([0-9][%w%.]*)') do
                if info[key] == nil then info[key] = tostring(val) end
            end
            return info
        end
    end
    -- 混淆兜底：DeltaPageInfo 变量名被重命名时，搜索顶格单行扁平表
    -- 匹配形如：var = { name="x", title="y", icon="z", version="1.0" }
    -- 字符串键不会被混淆器重命名，因此可靠
    do
        for line in (code .. "\n"):gmatch("([^\r\n]*)[\r\n]") do
            local body = line:match("^[%w_]+%s*=%s*{(.*)}%s*$")
            if body and body:find('name%s*=%s*"') and (body:find('title%s*=%s*"') or body:find('icon%s*=%s*"')) then
                local info = {}
                for key, val in body:gmatch('([%w_]+)%s*=%s*"([^"]*)"') do info[key] = val end
                for key, val in body:gmatch("([%w_]+)%s*=%s*'([^']*)'") do
                    if info[key] == nil then info[key] = val end
...
local function getInstalledPageVersion(name)
    local version = "1.0.0"
    pcall(function()
        local path = "DeltaUI/Pages/" .. name .. ".lua"
        if not isfile(path) then return end
        local content = readfile(path)
        if not content or #content == 0 then return end
        local info = extractPageInfo(content)
        if info and type(info.version) == "string" and info.version ~= "" then
            version = info.version
            return
        end
        local firstLine = content:match("^[^\n]*")
        if firstLine and firstLine:match("^%-%- DeltaUIPage:") then
            local v = firstLine:match("version=([^|]+)")
...
-- 预设页面的远端版本：拉取页面源码后读其中的 DeltaPageInfo.version
-- 只在「已安装」时探测，结果落盘缓存 6 小时，避免每次刷新卡片都整包下载
local presetVerCacheFile = "DeltaUI/Cache/PresetVersions.json"
local presetRemoteVer = {}
local presetVerLoaded = false
local presetVerProbing = {}

local function loadPresetVerCache()
    if presetVerLoaded then return end
    presetVerLoaded = true
    pcall(function()
        if not isfile(presetVerCacheFile) then return end
        local ok, data = pcall(function()
            return svc.HttpService:JSONDecode(readfile(presetVerCacheFile))
        end)
        if not ok or type(data) ~= "table" then return end
        local now = os.time()
        for name, entry in pairs(data) do
            if type(entry) == "table" and type(entry.version) == "string" and entry.version ~= "" then
                if type(entry.time) ~= "number" or (now - entry.time) < 6 * 3600 then
                    presetRemoteVer[name] = entry.version
                end
            end
        end
    end)
...
local function savePresetVerCache()
    pcall(function()
        if not isfolder("DeltaUI/Cache") then makefolder("DeltaUI/Cache") end
        local out = {}
        for name, ver in pairs(presetRemoteVer) do
            out[name] = { version = ver, time = os.time() }
        end
        writefile(presetVerCacheFile, svc.HttpService:JSONEncode(out))
    end)
end

local function probePresetRemoteVersion(preset)
    loadPresetVerCache()
    local name = preset.name
    if presetRemoteVer[name] or presetVerProbing[name] then return end
    presetVerProbing[name] = true
    task.spawn(function()
        local code = nil
        pcall(function() code = game:HttpGet(preset.url) end)
        if not code or #code == 0 then
            local ok, res = pcall(requestWithUA, preset.url)
            if ok and type(res) == "string" then code = res end
        end
        local info = extractPageInfo(code or "")
        presetVerProbing[name] = false
        if info and type(info.version) == "string" and info.version ~= "" then
            presetRemoteVer[name] = info.version
            savePresetVerCache()
...
for i, preset in ipairs(PRESET_PAGES) do
    ...
    local function isPageInstalled(name)
        local ok, result = pcall(function()
            return isfile("DeltaUI/Pages/" .. name .. ".lua")
        end)
        return ok and result
    end

    local function hasUpdate()
        -- 版本来源：预设里写死的 version 优先（用于强制锁定），否则用远端 DeltaPageInfo.version
        local remoteVer = preset.version or presetRemoteVer[preset.name]
        if not remoteVer then return false end
        local localVer = getInstalledPageVersion(preset.name)
        return compareVersions(remoteVer, localVer) > 0
    end

    local function refreshPresetBtn()
        if currentBtn then currentBtn:Destroy() end
        local installed = isPageInstalled(preset.name)
        if installed and not preset.version then
            probePresetRemoteVersion(preset)
        end
        ...
        elseif hasUpdate() then
            -- 有更新，显示绿色更新按钮
            local btn = create("TextButton", {AnchorPoint = Vector2.new(1, 0.5), Position = UDim2.new(1, -10, 0.5, 0), Size = UDim2.new(0, 72, 0, 30), BackgroundColor3 = theme.green, BackgroundTransparency = 0.3, Text = "", BorderSizePixel = 0, ZIndex = 5})
            corner(8, btn)
            local btnText = create("TextLabel", {Size = UDim2.new(1, 0, 1, 0), BackgroundTransparency = 1, Text = "更新", TextColor3 = Color3.fromRGB(255,255,255), TextSize = 12, Font = Enum.Font.SourceSansBold, ZIndex = 6})
            btnText.Parent = btn
            safeConnect(btn, "MouseButton1Click", function()
                -- 更新页面：先卸载再重新安装
                if pages[preset.name] then
                    uninstallExternalPage(preset.name)
                else
                    -- 只删文件，内存中没有加载
                    pcall(function()
                        if isfile("DeltaUI/Pages/" .. preset.name .. ".lua") then
                            delfile("DeltaUI/Pages/" .. preset.name .. ".lua")
                        end
                    end)
                end
                task.wait(0.2)
...
local function makePageHelpers()
    ...
    -- 自定义页面 API：获取 DeltaUI 版本号
    helpers.getUIVersion = function()
        return UI_VERSION
    end
    helpers.uiVersion = UI_VERSION
...
local function makeSafeEnv(frame, helpers)
    ...
    -- 自定义页面 API：Safe Mode 下同样可直接调用 getUIVersion()
    env.getUIVersion = helpers.getUIVersion
    env.uiVersion = UI_VERSION
...
function registerExternalPage(code, url, defOverride, forceUnsafe)
    -- 自动清理：加载失败时删除已安装的损坏页面文件（仅当调用方传入了 defOverride.name 时触发）
    local function autoRemoveCorruptPage(pageName)
        if not pageName or type(pageName) ~= "string" or pageName == "" then return end
        pcall(function()
            local p = "DeltaUI/Pages/" .. pageName .. ".lua"
            if isfile(p) then delfile(p) end
        end)
    end
    local cleanupName = (defOverride and type(defOverride) == "table" and defOverride.name) or nil
    ...
    local prevPageInfo = _G.DeltaPageInfo
    ...
    if not chunk then
        ...
        autoRemoveCorruptPage(cleanupName)
        if cleanupName then
            ShowNotification(t("page_install_failed") .. " (" .. cleanupName .. ")", 3)
        else
            ShowNotification(t("page_install_failed"), 3)
        end
        return false
    ...
    local env = nil
    _G.DeltaPageInfo = nil
    if safeMode then
        
        env = makeSafeEnv(frame, helpers)
    ...
    -- 页面代码变量 DeltaPageInfo：运行时值优先于静态解析，混淆后依然可取
    local pageInfoVar = (type(env) == "table" and env.DeltaPageInfo) or _G.DeltaPageInfo
    _G.DeltaPageInfo = prevPageInfo
    ...
    -- 页面信息以代码变量 DeltaPageInfo 为准：运行时值 > 静态解析源码 > 脚本 return/DeltaRegisterPage 的字段
    do
        local info = pageInfoVar
        if type(info) ~= "table" then info = extractPageInfo(code) end
        if type(info) == "table" then
            for _, key in ipairs(PAGE_INFO_FIELDS) do
                local v = info[key]
                if type(v) == "string" and v ~= "" then def[key] = v end
            end
            if type(def.dataFolder) == "string" then def.dataFolder = __safeFilterName(def.dataFolder) end
            local overrideName = defOverride and defOverride.name
            if (type(overrideName) ~= "string" or overrideName == "")
                and type(info.name) == "string" and info.name ~= ""
                and not pages[__safeFilterName(info.name)] then
                def.name = __safeFilterName(info.name)
            end
        end
    end
    ...
    if not builtOk then
        ...
        _G.DeltaPage = prevPage
        autoRemoveCorruptPage(cleanupName or name)
        local failName = cleanupName or name
        if failName then
            ShowNotification(t("page_install_failed") .. " (" .. failName .. ")", 3)
        else
            ShowNotification(t("page_install_failed"), 3)
        end
        return false
    ...
    pcall(function()
        ...
        local meta = string.format("-- DeltaUIPage: name=%s|title=%s|icon=%s|url=%s|unsafe=%s|version=%s\n", name, def.title or "", def.icon or "", url or "", forceUnsafe and "1" or "0", def.version or "1.0.0")
        writefile("DeltaUI/Pages/" .. name .. ".lua", meta .. code)
    end)
...
function installExternalPageFromURL(url, forceUnsafe, defOverride)
    ...
    return registerExternalPage(code, url, defOverride, forceUnsafe)
end
...
function uninstallExternalPage(name)
    ...
    pcall(function()
        fileExists = isfile("DeltaUI/Pages/" .. name .. ".lua")
    end)
    if not pages[name] and not fileExists then return false end
...
local function parsePageMeta(code)
    if not code or type(code) ~= "string" or #code == 0 then return nil end
    local firstLine = code:match("^[^\n]*")
    if not firstLine or not firstLine:match("^%-%- DeltaUIPage:") then return nil end
    local meta = {}
    for k, v in firstLine:gmatch("([%w_]+)=([^|]*)") do
        meta[k] = v
    end
    -- 去掉元数据行，返回纯代码
    local pureCode = code:sub(#firstLine + 2)
    return meta, pureCode
end

local function getInstalledPagesFromFiles()
    local result = {}
    pcall(function()
        if not isfolder("DeltaUI/Pages") then return end
        local files = listfiles("DeltaUI/Pages")
        if not files then return end
        for _, fpath in ipairs(files) do
            local fname = fpath:match("([^/\\]+)%.lua$")
            if fname and fname ~= "" then
                local ok, content = pcall(function()
                    return readfile(fpath)
                end)
                if ok and content and #content > 0 then
                    local meta, pureCode = parsePageMeta(content)
                    -- 代码变量优先于顶部标识行（标识行仍用于 url/unsafe 这类 DeltaUI 侧信息）
                    local info = extractPageInfo(content) or {}
                    if meta and meta.name then
                        table.insert(result, {
                            name = meta.name,
                        ...
                        -- 没有元数据的旧文件，用文件名（卸载按文件名删文件，name 不能取自变量）
                        table.insert(result, {
                            name = fname,
...
function loadInstalledPages()
    local installed = getInstalledPagesFromFiles()
    for _, p in ipairs(installed) do
        if p and p.name and not pages[p.name] then
            task.spawn(function()
                pcall(function()
                    if p.code and #p.code > 0 then
                        registerExternalPage(p.code, p.url, {name = p.name, title = p.title, icon = p.icon}, p.unsafe)
                    end
                end)
            end)
        end
...
function refreshInstalledPagesList()
    ...
    local allPages = getInstalledPagesFromFiles()
    local list = {}
    -- 过滤掉官方预设页面
...
-- 获取当前选项卡的完整内容（处理分页显示的情况）
function getCurrentTabFullContent()
    if not tabs[currentTab] then return "" end
    saveCurrentTab()
    return tabs[currentTab].content or ""
end
_G.__DeltaUI_getCurrentTabFullContent = getCurrentTabFullContent

-- 获取所有选项卡的内容
function getAllTabsContent()
    local result = {}
    for idx, tab in ipairs(tabs) do
        if idx == currentTab then
            saveCurrentTab()
        end
        result[idx] = {
            name = tab.name,
...
_G.__DeltaUI_getAllTabsContent = getAllTabsContent

-- 供外部页面（如反混淆工具）在 addTab() 后设置当前代码页（顶部卡片）的标题
function setCurrentTabName(name)
    if not tabs[currentTab] then return end
    if type(name) ~= "string" or name == "" then return end
    tabs[currentTab].name = name
    if renderTabs then renderTabs() end
end
_G.__DeltaUI_setCurrentTabName = setCurrentTabName
