...
function pageDef.build(frame, helpers)
    ...
    -- codingPage 由外部页面系统提供
    -- codingPage 已由外部页面系统父化
    ...
    CODING_RIGHT_W_DEFAULT = 280   -- 常态宽度(像素)，未进入「添加积木」时的窄态（已收窄）
    CODING_RIGHT_W_COLLAPSED = CODING_RIGHT_W_DEFAULT  -- 收起态宽度(点击「添加积木」前)，与默认一致
    CODING_RIGHT_W_EXPANDED = 480  -- 进入添加模式后的宽度(像素)，向左吃掉左侧预览区
    CODING_ANIM_DUR = 0.28

    codingPickerOpen = false  -- 初始为「未展开」状态，与 codingSetPickerOpen(false) 一致

    function codingGetAvailableWidth()
    ...
    function codingApplyRightWidth(targetW, dur)
    ...
    function codingApplySearchWidth(fixedW)
    ...
    function codingVisibleOnScreen(obj)
        local o = obj
        while o do
    ...
    -- 屏幕点是否落在某个控件的矩形内
    function codingPointInObj(obj, px, py)
    ...
    function codingPointerBlocked(px, py)
    ...
    CODING_FADE_SKIP = {UIStroke = true, UICorner = true, UIGradient = true, UIPadding = true, UIListLayout = true, UIGridLayout = true, UITextSizeConstraint = true}
    ...
    function codingResetHoverTintIn(root)
        if not root then return end
        pcall(function()
    ...
    CODING_CARD_PURPLE = Color3.fromRGB(139, 92, 246)
    ...
    codingSavedGridProps = nil -- 保存被移除网格的原始属性，供重建时还原
    ...
    codingCardWaitLayoutStable

    -- 网格单元格常量（与下方 UIGridLayout 的 CellSize / CellPadding 保持一致）。
    -- 提前定义是因为 codingFreezeGridSort 也需要按行列把卡片位置落盘。
    CODING_CELL_H = 88              -- 单元格高
    CODING_CELL_PAD = 8             -- 单元格间距
    CODING_CELL_COLS = 2            -- 列数（CellSize.X = 0.5,-4 ⇒ 恒为 2 列）

    function codingSaveGridProps(grid)
    ...
    function codingWaitAllTweens(tweens, callback)
        local pending = 0
    ...
    function codingSyncCardsCanvasX()
    ...
    CODING_CARD_TOP_SAFE = 56        -- 顶部安全边距（与 codingPickerCards.Position.Y 一致）
    CODING_CARD_MARGIN = 8          -- 展开矩形到面板四边的留白
    ...
    function codingCardExpandedRect(host)
    ...
    function codingCardCellLocal(card)
    ...
    function codingCardCellRect(card)
        local cont = codingPickerCards
    ...
    function codingCardReturnToCell(card)
    ...
    function codingEnsureCardVisible(card)
        if not (card and codingPickerCards) then return end
        pcall(function()
    ...
    if codingEnterBtn then
        ...
        codingEnterBtn.MouseButton1Click:Connect(function()
            ...
            resetToBase()
    pcall(function() enterBuildSpace() end)
        end)
    ...
    csHeaderIcon = GetIcon("sliders-horizontal", UDim2.new(0, 15, 0, 15), theme.accent)
    ...
    csExitIcon = GetIcon("log-out", UDim2.new(0, 13, 0, 13), theme.red)
    if csExitIcon then
    ...
    csList = create("UIListLayout", {SortOrder = Enum.SortOrder.LayoutOrder, Padding = UDim.new(0, 10)})
    ...
    csCardList = create("UIListLayout", {
    ...
    csCardHeader = create("Frame", {Size = UDim2.new(1, 0, 0, 22), BackgroundTransparency = 1, LayoutOrder = -1, ZIndex = 14})
    csCardHeader.Parent = csCard
    csCardAccent = create("Frame", {Size = UDim2.new(0, 3, 0, 14), Position = UDim2.new(0, 2, 0.5, -7), BackgroundColor3 = theme.accent, BorderSizePixel = 0, ZIndex = 15})
    applyGradient(csCardAccent, theme.accent, theme.accent2, 90)
    corner(2, csCardAccent)
    csCardAccent.Parent = csCardHeader
    csCardIcon = GetIcon("sliders-horizontal", UDim2.new(0, 15, 0, 15))
    if csCardIcon then
    ...
    csPreviewBtn, csPreviewGet = makeDropdown(csPreviewRow, CODING_PREVIEW_TYPES, 1, function() end, "codingPreviewType")
    pcall(function()
...
local ok = register()
if not ok then
    warn("[DeltaUI CodingBlocks] 页面注册失败：未找到 DeltaRegisterPage，请确保在 DeltaUI 中加载此页面")
end
